...
function showPetQuote() {
  const bubble = document.getElementById('pet-bubble');
  const quotes = [
    "¡Grrr! El elo no se va a subir solo.",
    "Huelo miedo en el equipo enemigo... o es mi cena.",
    "¿Has visto mis Naafiri Coins? Juraría que estaban aquí.",
    "¡Subamos de rango! Mis colmillos están listos.",
    "¿Otra derrota? Tranquilo, yo te protejo el MMR.",
    "¡WOOF! Esa última partida fue legendaria.",
    "Dato curioso: El 100% de los perros ganan si les das galletas."
  ];
  ...
  setTimeout(() => bubble.classList.remove('pet-bubble--show'), 4000);
}

function updatePetMood(type) {
  const mood = document.getElementById('pet-mood');
  const bubble = document.getElementById('pet-bubble');
  if (!mood) return;

  if (type === 'rank_up') {
    mood.textContent = "¡Eufórico!";
    bubble.textContent = "¡SIII! ¡Ese es mi dueño!";
  } else if (type === 'rank_down') {
    mood.textContent = "Gruñendo";
    bubble.textContent = "No pasa nada, la próxima los destrozamos.";
  }
  
  bubble.classList.add('pet-bubble--show');
  setTimeout(() => bubble.classList.remove('pet-bubble--show'), 5000);
}